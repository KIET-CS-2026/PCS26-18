...
const GOOGLE_CLIENT_ID =
  "717432159606-7l81pdv9q543updjjjsd8tnlj5chsrrn.apps.googleusercontent.com";
      ...
      .then(() => {
        ...
        window.google.accounts.id.initialize({
          ...
          callback: (response) => {
            try {
              ...
              reject(
                new Error(`Failed to parse Google response: ${error.message}`)
              );
            }
          },
        ...
        window.google.accounts.id.prompt((notification) => {
          if (notification.isNotDisplayed()) {
            reject(
              new Error("Google Sign-In popup was blocked or not displayed")
            );
